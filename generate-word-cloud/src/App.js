...
class App extends Component {
  ...
  renderChart() {
    ...
    const centerY = 90;
    const svg = d3.select("svg").attr("width", 1000).attr("height", 150);
    ...
    const wordSpacing = d3
      .scaleLinear()
      .domain([0, data.length - 1])
      .range([150, 900]);

    const cloudBox = svg.selectAll("text").data(data, (d) => d[0]);

    cloudBox.exit().remove();

    cloudBox
      .transition()
      .duration(3000)
      .attr("x", (d, i) => wordSpacing(i))
      .attr("y", centerY)
      .style("fill", "black")
      .style("opacity", 1)
      .style("font-size", (d) => `${fontSize(d[1])}px`);
